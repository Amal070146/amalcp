...
def db_ground_booking(request):
    if request.method == 'POST':
        ...
        try:
            book = Book_ground.objects.filter(name=name)
            if book is not None:
            ...
            print("except block")
            user = User.objects.filter(name=request.session['uname'])

            book = Book_ground(uid=user.uid, name=user.name, date=date, time=time)
            book.save()
            request.session['book_status'] = "Booking successful"
            return user_home(request,'user_home.html')
